...
public class PostController {
    ...
    @GetMapping("/{id}")
    public PostResponse findById(@PathVariable Long id, HttpSession session) {
        return postService.findById(id, session);
    }

    @PatchMapping("/{id}")
    public PostResponse update(@PathVariable Long id, @RequestBody UpdatePostRequest request, HttpSession session) {
